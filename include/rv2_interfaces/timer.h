...
inline std::shared_ptr<LiteTimer> make_shared_timer(double period_ms, const std::function<void()>& func, bool MAX_SCHED_RR = false)
{
    return std::make_shared<LiteTimer>(period_ms, func, MAX_SCHED_RR);
}

inline std::unique_ptr<LiteTimer> make_unique_timer(double period_ms, const std::function<void()>& func, bool MAX_SCHED_RR = false)
{
